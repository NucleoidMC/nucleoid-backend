...
pub async fn send(discord: &mut DiscordClient, send_ping: SendPing) {
    if let (Some(cache_and_http), Some(data)) = (discord.cache_and_http.clone(), discord.data.clone()) {
...
impl Handler {
    pub async fn add(&self, ctx: &SerenityContext, message: &SerenityMessage, ping: &str, role_id: &str) -> CommandResult {
    ...
    pub async fn remove(&self, ctx: &SerenityContext, message: &SerenityMessage, ping: &str) -> CommandResult {
        let mut data = ctx.data.write().await;
        let ping_store = data.get_mut::<StoreKey>().unwrap();
    ...
    pub async fn disallow_for_role(&self, ctx: &SerenityContext, message: &SerenityMessage, ping: &str, role: &str) -> CommandResult {
        let role = role.parse::<u64>().map_err(|_| CommandError::InvalidRoleId)?;
    ...
    pub async fn request(&self, ctx: &SerenityContext, message: &SerenityMessage, ping: &str) -> CommandResult {
